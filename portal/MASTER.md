...
## Changelog
### 2026-07-14 (latest+++++++++++++++) — Landed Cost: tabular view matches graph height + section spacing
...
- **Section spacing:** added a `_space(px)` helper (thin `<div>` spacer) and inserted breathing room after every
  section in `calc_import_price.render()` (top banner, graph/vars row, scenario inputs, sensitivity, PDF,
  methodology, glossary) around the existing dividers.

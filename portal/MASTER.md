...
- `accuracy_tables/Accuracy_Table_11.xlsx` (**1 week ahead**) + `accuracy_tables/Accuracy_Table_6_4W.xlsx` / `_8W` / `_12W` (**4 / 8 / 12 weeks ahead**, added 2026-08-07) — week-wise Actual/Forecast (sheet **`Ensemble_WgtMean`** only; the 4/8/12W workbooks also carry NHITS / NBEATSx / PatchTST / Ensemble_Median sheets, which the app ignores), one wide 7-col block per product; parsed by column position (`_read_accuracy` finds each block by its product label in row 0). **Identical layout across all four**, so one parser serves them all — only the sheet's own lag differs (`B{r-1}` vs `B{r-4/8/12}`). `ACC_FILES` is keyed by **horizon int** (`1/4/8/12`); every `load_accuracy(window, …)` / `accuracy_averages(…, window)` call passes one. Table_11 was renamed from `Accuracy_Table_6.xlsx` on 2026-07-10 (6/16-week both retired). Stored MAE/MAPA/Delta/Directional cols are sparse → derive Delta/Direction from Actual/Forecast (Up/Down/Flat via `direction_flag`, ±500 Rs./ton dead-band ⇒ Flat).
  > ⚠ Those metric columns are **Excel formulas**. openpyxl/pandas read only *cached* values, so a workbook written by a script and pushed **without being opened + saved in Excel** has `None` there → blank MAPA/Directional/Delta KPIs and blank weekly-accuracy charts (Actual/Forecast still plot). **Open each new/regenerated accuracy table in Excel and save it before pushing.**
...
- ⚠ **Data-side action**: the three new files (and the locally-modified `Accuracy_Table_11.xlsx`) are still **uncommitted** in `dashboard-data`, and their MAPA/Delta/Directional **formula cells have no cached values** — open each in Excel, save, then commit/push, or those KPIs and the three accuracy charts render blank. See the ⚠ note under Data sources.

...
- Expanded the analyst tour ([tour.py](portal/tour.py) `STEPS`) from a single "whole page" step per section to **35 granular steps**. Each cross-page section now highlights its individual controls:
  ...
  - **Methodology** — hero (`.bm-meth-hero`), stat strip (`.bm-stat-row`), the "From data to forecast" Inputs→model→Outputs infographic (`.bm-engine`), the "Forecasting Model" modular pipeline (`.bm-fm`), key-factor grid (`.bm-factor-grid`), horizon block (`.bm-horizon`).

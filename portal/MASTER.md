...
## Changelog
### 2026-07-18 (latest+++++++++++++++++++++++++++++++++++) — Login + reset pages redesigned from scratch (modular, self-contained branded card)
- [app.py](portal/app.py) `login_screen()` + `force_password_change()` rebuilt. New **self-contained branded card** (no more topbar on these pages): BigMint logo + product title + subtitle in a `_login_brand(title, sub)` header, a thin blue→orange brand strip across the card top, soft shadow. **Auth engine unchanged** (`auth.authenticate` / `_start_session` / status handling identical).

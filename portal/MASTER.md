...
- **App-wide input border-box fix** — added a `theme.py` rule giving **stNumberInput / stTextInput / stTextArea / stDateInput** the same clean box as the dropdowns (white fill + ONE rounded orange border, no inner notch/seam, orange −/+ stepper glyphs). Fixes the pale/double-border look on the un-scoped inputs (Price Sensitivity "Current price", Cost Head market price, admin/login fields). Colour-only border (never width). Scoped knob-card CSS is more specific and still wins.

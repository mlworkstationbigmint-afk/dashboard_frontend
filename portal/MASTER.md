...
## Changelog
### 2026-07-11 (latest) — Performance KPI: "Average delta" → 12-week directional hit rate
- **Third KPI card replaced.** The Performance dashboard's k3 card was **"Average delta"** (`avg_delta` = mean of `DeltaPct`); it's now **"Directional hit rate (12 wk)"** — the count of *correct* weekly directional calls over the **last 12 weeks ÷ 12**, shown as a percentage (`{:.0f}%`), sublabel "correct calls / last 12 weeks". → `app.py` `page_performance()` k3 + `data_loader.accuracy_kpis()`.

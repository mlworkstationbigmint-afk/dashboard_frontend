...
- Title: plain-text "Price Forecasting: Steel" (browser tab / login caption / Home H2). The **topbar heading** is now **one uniform title on every role + login** (2026-07-22): **"GCP Steel - AI Labs - BigMint - Steel Price Forecasting Model"** — set once on `DEFAULT_PROFILE["title"]`; `ROLE_PROFILES` entries no longer carry a `title` (they inherit it). Layout = **co-brand chip · pipe · BigMint logo · pipe · title** (Adani chip sits BEFORE the BigMint logo since 2026-07-22; separators are pipes, not `×`). Login + Adani show the Adani chip; internal **Analyst/Admin** profiles have `cobrand_logo=None` so they drop the chip (BigMint logo + title only). Brand name is **BigMint** (never "Bigmint").
...
- **Performance dashboard** — product selector + a **forecast-horizon pill strip (1W / 4W / 8W / 12W)**, each horizon reading its own accuracy table (`dl.ACC_FILES`); reads **all rows** of the picked table. MAPA / directional / avg-delta KPIs, actual-vs-forecast chart + weekly-delta (Rs.) bars + **weekly accuracy % line** + **weekly directional-accuracy bars** + week-wise table. **All four tables carry all 11 products** since 2026-09-03, when the Mundra forecast columns missing from the head of the 4/8/12W tables were filled — so the "No N-week-ahead accuracy data for this product" branch no longer fires for Mundra. It is still reachable and still worth keeping: the 4/8/12W tables have no Delta/Directional on their first *h* rows (those cells reference row `r-h`, which does not exist).
...
- **China import-parity landed-cost line (violet overlay)** → weekly duty-paid landed cost read from `landed_costs.xlsx` (one sheet per product, `Date` + `Landed cost India (INR/t)`, ex-IGST, dated to week-end) via `data_loader.load_landed(sheet)`; drawn by `forecast_chart(landed=…)`; colour `CHINA_LANDED_LINE` (just above `forecast_chart`). Which products get it → `page_forecasting()` `_LANDED_OVERLAY = {product: (sheet, label)}` (currently HRC, Rebar IF Mumbai, HR Plate). Add a product = add its sheet to `landed_costs.xlsx` + one entry here.
...
- Data parsing → `data_loader.py`. **Accuracy metrics (MAPA / Delta / Directional)** → read from the sheet, blanks filled by `_sheet_metrics()` (the sheet's Excel formulas re-run in pandas, `lag` = the horizon); the horizon→file map is `ACC_FILES`. Self-check: `python portal/data_loader.py`
- Data **location** → `data_loader.py` `_data_root()`: fetched private-repo temp dir when `st.secrets['data']` is set (`_fetch_private_data_dir`, cache-keyed on `_remote_sha` so a push auto-refetches), else the in-repo sample (repo root). `acc_dir()` / `calculators_csv()` / `ff_path()` / `acc_path()` build every path from that root. Swap backend (S3/GCS) = rewrite `_fetch_private_data_dir()` body
- Direction Up/Down/Flat + flat threshold (500) → `data_loader.py` `direction_flag()` / `FLAT_THRESHOLD`

## Data sources (read once, cached)
> **Location:** resolved by `data_loader._data_root()` — a **private GitHub repo** (downloaded to a temp dir) when `st.secrets['data']` is configured, else the **in-repo sample**. `acc_dir()` / `calculators_csv()` build paths from that root; readers are **mtime-keyed** (`@st.cache_data`). The layout below is the same in the private repo and the sample. **Private-repo edits auto-show now (2026-07-24):** `_data_root` keys the fetch on the repo HEAD SHA (`_remote_sha`, ~25s cache) and `data_signature()` returns that SHA, so `_auto_refresh_on_data_change` (30s poll) sees a push and re-fetches on its own — no reboot/clear needed. Push to the data repo (`main`) and it appears within a poll cycle. **On every fresh page load / login** (empty `session_state`), `app.py` clears `_remote_sha` (`if "_booted" not in session_state`) so the HEAD is re-checked immediately and the newest push is fetched — the per-SHA download is reused when the repo hasn't moved, so an unchanged repo is one cheap API call, not a re-download. (The sidebar "Refresh data" button is hidden by `theme.py` CSS, so page-load/poll is the whole refresh story; the button still `st.cache_resource.clear()`s if ever unhidden.)
...
- `accuracy_tables/Accuracy_Table_11.xlsx` (**1 week ahead**) + `accuracy_tables/Accuracy_Table_11_4W.xlsx` / `_8W` / `_12W` (**4 / 8 / 12 weeks ahead**, added 2026-08-07) — week-wise Actual/Forecast (**one sheet, `Ensemble_WgtMean`**, in all four; the 4/8/12W workbooks used to also carry NHITS / NBEATSx / PatchTST / Ensemble_Median, which the app ignored — **dropped 2026-09-03**, which cut those three files ~70%), one wide 7-col block per product; parsed by column position (`_read_accuracy` finds each block by its product label in row 0). **Identical layout across all four**, so one parser serves them all — only the sheet's own lag differs (`B{r-1}` vs `B{r-4/8/12}`). `ACC_FILES` is keyed by **horizon int** (`1/4/8/12`); every `load_accuracy(window, …)` / `accuracy_averages(…, window)` call passes one. Table_11 was renamed from `Accuracy_Table_6.xlsx` on 2026-07-10 (6/16-week both retired). Stored MAE/MAPA/Delta/Directional cols are sparse → derive Delta/Direction from Actual/Forecast (Up/Down/Flat via `direction_flag`, ±500 Rs./ton dead-band ⇒ Flat).
  > ⚠ **The MAPA / Delta (%) / Directional (%) cells — and the AVERAGE row — are Excel FORMULAS.** pandas/openpyxl can only read a formula's *cached* result, so a workbook generated by a script and never opened+saved in Excel reads **blank** everywhere. That is what emptied the Performance KPIs and the two accuracy charts on 2026-08-07. **`data_loader._sheet_metrics()` now re-runs those formulas in pandas** (verified cell-for-cell against the cached values in every product block of Table_11, max diff 4e-16) and fills any blank; a cached value, when present, still wins. So the app no longer depends on the file having been opened in Excel — but a **stale** cache does still win over a fresh recompute (see the note in the 2026-08-07 changelog entry).
...
- **The five Mundra products now have their own entries** — they no longer fall back to `_default`. They read as standalone Kutch/west-coast market commentary: **the derivation is deliberately NOT stated in the rendered text** (no "tracks Mumbai", no ratios/offsets, no "there is no separate price discovery") — user instruction, 2026-09-11. Keep it that way when refreshing.
- For maintenance only, the bases carried in `forecast_forward.xlsx` are: HRC Mundra = HRC **+₹450/t**, HR Plate Mundra (20-40mm) = HR Plate **+₹750/t**, Rebar BF Mundra = Rebar BF Mumbai **×1.03553**, Rebar IF Mundra = Rebar IF Mumbai **×1.06744**, Structure Mundra = Structure IF Raipur **+₹1,300/t**. Mundra-specific content (Khavda/Kutch renewable + transmission pipeline, Mundra as a first-landfall import gateway, AM/NS Hazira 9→15 mt) is from web research — the monthly document has no Mundra series.
...
- **All four accuracy tables now end at week 2026-08-30** (was 2026-08-23), 2024-01-07 onward:
  1W **138 rows**, 4W / 8W / 12W **139 rows**. Actuals come from the rebuilt weekly_forecast master
  dataset; forecasts are read out of `<product>_cv.csv` at `cutoff = t - h`, `ds = t`, so no week is
  re-forecast.
- **Coverage is now all 11 products on every horizon.** The Mundra forecast columns were blank for
  the first *h* rows of the 4/8/12W tables (4 / 8 / 12 rows × 5 products = 120 cells) because the
  rule is `base forecast(h) + spread(t-h)` and `t-h` fell before the table started. The spread is a
  location premium, not a forecast, so it is flat-extended backwards from row 4. **Delta and
  Directional stay blank on those first *h* rows** — they reference row `r-h`, which does not exist,
  exactly as the base products' own Delta/Directional do there. Delta-scored weeks are therefore
  `rows - h`: 1W 137, 4W 135, 8W 131, 12W 127.
- **Directional-hit blocks re-laid bottom-anchored** so the **last block is always exactly 12 rows**
  and only the **first** carries the remainder (1W 6 rows, the others 7). They had been top-anchored
  from row 4, which made the trailing block — the one the current reading comes from — the only one
  never a full 12, growing by one every week. The vertical merges were re-laid with them; they had
  drifted out of step (`H136:H139` merged under a `G136:G140` COUNTIF). Formulas were retargeted from
  each block's existing `=COUNTIF(G…,1)&"/"&COUNT(G…)` template, not rewritten: 12 per product before
  and after.
- **The NHITS / NBEATSx / PatchTST / Ensemble_Median sheets were dropped** from the 4/8/12W workbooks;
  every table is now a single `Ensemble_WgtMean` sheet. The app only ever read that one
  (`data_loader.HEADLINE_SHEET`), and nothing references the others. 556→159 KB, 537→156 KB, 516→153 KB.
- **`forecast_forward.xlsx` rebuilt** anchored on 2026-08-30, horizon **2026-09-06 → 2026-11-22**;
  11 product sheets + Summary, sheet order, headers, damping and the ±500 dead band unchanged. `F`/`G`
  are now consistently NHITS then NBEATSx on all 11 sheets — they were swapped on the two STRUCTURE
  sheets (harmless, since cols A–E are all the frontend reads).

### 2026-08-10 — Landed-cost overlay refreshed to Wk 32-26 off new FOB assessments
- **`accuracy_tables/landed_costs.xlsx` in the private `dashboard-data` repo updated** (all 3 sheets — HRC / Rebar / HR Plate) from the BigMint weekly assessments for **Wk 24–32 of 2026** (week-end 14-Jun-26 → 9-Aug-26). Frontend code unchanged: `data_loader.load_landed()` still reads Date + col B, and the `_LANDED_OVERLAY` violet line on the HRC / Rebar IF Mumbai / HR Plate forecast charts now runs 3 weeks further.
- Recomputed with the **same pipelines the sheets were originally built from**, re-derived from `Downloads\Landed Costs 1.xlsx` (HRC + Donghua rebar) and `Downloads\HRPlate_China_landed_cost (1).xlsx`, then verified against every existing row before writing: Rebar 132/132 rows reproduced exactly, HR Plate exact from Oct-25 onward, HRC 133/134 (one 2024 week off by ₹100 — an upstream FOB revision, left alone). Rounding is `ROUND(x/50)*50`, as the existing series uses.
- **Rebar** (EXW Donghua CNY/t → +₹80 inland → CIF → BCD 15% + SWS + ₹900 port): Wk 29 revised **3180 → 3208** (₹57,700 → **₹58,200**); new Wk 30/31/32 = 3212 / 3186 / 3130 → ₹58,250 / ₹57,800 / ₹56,900.
- **HRC** (FOB Rizhao USD/t from `calculators/HRC.csv`, already refreshed through 9-Aug-26): Wk 24–29 already matched, so only Wk 30/31/32 appended at FOB $495 → **₹60,950** each. Sheet's source note corrected — it still claimed *"Rizhao price series ends 25-Jan-2026"*.
- **HR Plate** (FOB Rizhao $540 flat across Wk 24–32): Wk 24–29 unchanged; Wk 30/31/32 appended at **₹66,050** each.
- **Carried forward** for the three new weeks (07-26 / 08-02 / 08-09), since the source workbooks' rate tables also stop at 19-Jul-26: CN→India freight $24.3/t, USD/INR 96.261, CNY/INR 14.19678 (HR Plate: freight $25.6/t, USD/INR 96.659). Same carry-forward convention the freight sheet itself uses. Replace with real rates when the workbooks are next refreshed.
- The two source workbooks in `Downloads\` were **not** extended — only the portal-facing `landed_costs.xlsx`.

### 2026-08-10 — Price Sensitivity: knob cards show "as of" date + absolute-value entry
- **`driver_control()` number box now takes an absolute value** ([calc_elasticity.py](portal/calculators/calc_elasticity.py)), not a %  — e.g. type `6500` (₹/t) instead of a shock %; converted to the canonical %-shock via the driver's baseline (`_cb_number`), which still drives the slider/model exactly as before. Range/step mirror the slider's ±50% (`baseline*0.5..baseline*1.5`, ~0.5%-sized steps).
- **Added a "Prices as of {date}" caption to each knob card** — `_load_model()`/`load_model()` now also return the CSV's latest `Date` (`.max()`), threaded through `_hrc_spec()["as_of"]` → `driver_control(..., as_of=...)`. HR Plate/Rebar (fixed-sensitivity, non-CSV) don't pass it, so no caption shows there.

### 2026-08-10 — Price Sensitivity: HRC dataset refreshed to Aug-26 + date-sort bug fix
- **`calculators/HRC.csv` in the private `dashboard-data` repo replaced** (710 weekly rows, 6-Jan-13 → 9-Aug-26; was capped 25-Jan-26) — sourced from the user's `Weekly_Mastersheet_AutoRef.xlsx` → `HRC` sheet, which already carries the exact 11 series names `calc_elasticity.py` expects (target + 10 features), so no substitute columns were needed. Historical values for a few series (Iron Ore Odisha, HRC FOB Rizhao, CRC, the HRC target itself) differ from the old file for the same old dates — the mastersheet is a revised/rebased vintage, not a merge-on-top; treated as a full replace per the update request.
- **Fixed a live bug in `_load_model()`** ([calc_elasticity.py](portal/calculators/calc_elasticity.py)): `df.sort_values(by="Date")` sorted the `d-Mon-yy` strings **lexically**, not chronologically — every return (`log().diff()`) was computed between the wrong pairs of weeks, and `latest_price` silently locked onto whichever date sorted last alphabetically (`9-Sep-18`, giving a stale ₹46,000 baseline regardless of how current the CSV was). Now parses `Date` via `pd.to_datetime(..., format="%d-%b-%y")` before sorting. Verified: `latest_price` now resolves to the true latest row (₹57,950, 9-Aug-26); Ridge R² dropped from a bogus 0.98 (fit on shuffled dates) to a realistic ~0.06.

### 2026-08-10 — Home KPIs: horizon-list labels + per-horizon MAPA string
- **`page_home()` KPI strip** ([app.py](portal/app.py)): "Steel products" → **"Steel Products variants across locations"**; "Forecast horizon" value `"12 wk"` → **`"1W/4W/8W/12W"`** (built from `dl.ACC_FILES` keys); "Avg absolute accuracy" label → **`"Avg absolute accuracy [1W/4W/8W/12W]"`**, value is now `dl.accuracy_averages(meta["acc"], window=w)` averaged per product **per horizon**, joined `"/"` (e.g. `"98.9/97.1/94.9/92.2%"`) instead of a single 1W-only number.

### 2026-08-07 — Performance: 1W / 4W / 8W / 12W horizon pills wired to the new accuracy tables
- **`ACC_FILES` re-keyed by horizon** ([data_loader.py](portal/data_loader.py)): `{1: Accuracy_Table_11.xlsx, 4: Accuracy_Table_6_4W.xlsx, 8: …_8W, 12: …_12W}` (was the single `"11-week"` key). `acc_path`/`load_accuracy`/`accuracy_averages` now take an **int** horizon (`accuracy_averages` defaults to `1`); the three old `load_accuracy("11-week", …)` callers ([app.py](portal/app.py) Home KPIs + forecasting history, `last_actual_date`) pass `1`. Sheet stays **`Ensemble_WgtMean`** — the new workbooks' NHITS/NBEATSx/PatchTST/Ensemble_Median sheets are ignored.
- **Fetch + degrade**: `_fetch_private_data_dir` marks the 4/8/12W files **optional** (a `404` skips them instead of failing the whole fetch and silently dropping the app to the sample), and `load_accuracy`/`accuracy_averages` swallow a missing file/sheet → empty frame / all-`None` KPIs. So the code can ship before the data repo has the files.
- **Horizon pill strip on the Performance page** ([app.py](portal/app.py) `page_performance`): `st.segmented_control("Forecast horizon (weeks ahead)", [1,4,8,12], format_func=…W, key="perf_horizon")` under the product picker; KPIs, all five charts and the week-wise table read the picked horizon. Styling reuses the forecasting page's strip — [theme.py](portal/theme.py) `.st-key-fc_horizon` rules now also list `.st-key-perf_horizon` (light-blue 1W → dark-blue 12W gradient, orange fill on the active pill).
- **Coverage** *(as of 2026-08-07 — **superseded**, see the 2026-09-03 entry)*: 1W = all 11 products (134/131 wks); 4W = 79 wks, 8W = 75, 12W = 71 — the latter three only carry the **6 Mumbai/Raipur products**, so Mundra + those horizons shows the "No N-week-ahead accuracy data" warning.
- **Accuracy metrics no longer depend on Excel having cached the formulas** — new `_sheet_metrics(actual, forecast, lag)` in [data_loader.py](portal/data_loader.py) re-runs the sheet's own MAPA / Delta / Directional formulas in pandas (same ±500 dead-band, reference = the actual **`lag` weeks back**, i.e. the horizon), and `_read_accuracy` `fillna()`s the sheet's columns with it **before** any row is dropped (row positions must line up with the sheet's `B{r-lag}` references). `accuracy_averages` falls back the same way: a blank AVERAGE cell → the mean of that week-wise column. **This was the bug** — the served workbook was script-generated and never opened in Excel, so every metric read blank: KPIs showed "-", every directional bar showed "Wrong" (`Hit = DirAcc == 1.0` on an all-NaN column) and the delta-accuracy chart was empty. Guarded by a `python portal/data_loader.py` self-check (one row per formula branch).
- **Verified against Excel**: recomputed vs the cached values in the committed `Accuracy_Table_11.xlsx` — all 11 products × 3 metrics match to **4e-16**, identical blank patterns. Sanity: accuracy decays with horizon (HRC MAPA 98.9 → 97.1 → 94.9 → 92.2; delta 67 → 37 → 30 → 26).
- ⚠ **Two stale-cache findings in the committed Table_11** (pre-existing, unchanged by this work — the cached value still wins where present): its **AVERAGE row disagrees with its own column** (HR PLATE directional: row says 47.3%, the column means 48.1%) and `REBAR BF MUMBAI` row 136's Delta caches `0.3846` where its own formula on the current numbers gives `0`. Both mean the workbook was edited after its last Excel recalculation. Re-saving it in Excel resolves both; flipping `_read_accuracy` to prefer the recompute over the cache would too (one `fillna` → assignment) — **not done**, since "the sheet's formulas are the source of truth" is a standing decision.

### 2026-07-25 — Cost Head IF route: new heads (Pellet/Coal rb2) + editable per-plant metallic mix
- **`IF_ELEMENTS` rebuilt** ([calc_cost.py](portal/calculators/calc_cost.py)) to the new IF cost heads: **Pellet 1.83**, Scrap HMS 80:20 1.05, Pig Iron 1.04, Ferroalloys (SiMn) 1.00, **Coal rb2 0.90**, Electricity **1030**, Processing/Misc/Finance/Depreciation 1. For a **metallic** row (Pellet/Scrap/Pig/Ferroalloys) the norm shown IS its **yield**; per-plant consumption norm = **yield × mix share**. Non-metallic rows (Coal/Electricity/OpEx) use the norm directly. (Sponge Iron → Pellet.) ⚠ **Pellet ₹11,000 & Coal rb2 ₹12,000 unit prices are placeholders** — no price given; editable in-table.
- **`IF_YIELD` deleted** — the yield now lives in the element's own norm (single source), so `_seed_df` uses `float(norm) × mix[label]`. `_mix_note` footnote removed.
- **`IF_MIX` now includes Ferroalloys (0.012)** per plant: Durgapur = Pellet 80 / Scrap 15 / Pig 5 / Ferro 1.2; Jalna = Pellet 20 / Scrap 75 / Pig 5 / Ferro 1.2 (shares kept from before; Sponge→Pellet).
- **New editable per-plant metallic-mix mini table** below each IF plant's cost table (`_mix_seed_df`/`_mix_shares`/`_mix_editor`), with its own small **↺ Reset mix** button (own `mixver_{key}_{plant}` version, so it re-seeds independently of the main Reset). Edits flow into that plant's metallic norms live (fragment rerun).
- **Admin can edit the mix defaults** — new `_admin_mix_editor()` in `render_admin_defaults()` (below the cost-head editor): one mini table per IF plant + **Save as default** / **Reset to built-in**, persisted under new DB key **`if_mix_defaults`** (`IF_MIX_KEY`) via `_if_mix()` (admin-saved overrides built-in `IF_MIX`). Cost-head defaults still flow the same way (put the same list in admin = automatic, admin IF editor reads `_elements("IF")`).
...
### 2026-07-24 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Delta accuracy drop "negative" clause + revert deviation note to subheading
- **Delta accuracy definition trimmed** ([app.py](portal/app.py) `ACC_GLOSSARY["delta"]`): dropped "; a negative value indicates the opposite direction." — single source, so gone from the ⓘ hover on "Weekly delta accuracy", the glossary box, and the Delta-accuracy KPI card.
- **Deviation-graph note reverted out of the title row** ([app.py](portal/app.py) `page_performance`): back to `theme.section_title(...)` heading + a `bm-footnote` subtext line under it (`margin-top:-4px`) — not bold, a bit smaller, no longer inline inside the heading (undoes the prior flex-row-beside-heading change).

### 2026-07-24 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology manual button to the RIGHT of hero text
- **Manual button moved beside the text (not below).** `div[class*="st-key-meth_hero"]` in [theme.py](portal/theme.py) now `flex-direction:row; align-items:center; gap:24px`; first element container (text) `flex:1 1 auto; min-width:0`, last (button) `flex:0 0 auto; width:auto`. Button stays nested inside the blue card, now vertically centred on the right. CSS-only.

### 2026-07-24 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology hero button-in-card + Performance note-beside-heading + delta hover revert
- **Manual button now INSIDE the blue hero.** Hero text + `st.download_button` wrapped in one `st.container(key="meth_hero")`; the blue-card box styling (gradient/radius/padding/margin/shadow) moved from `.bm-meth-hero` onto `div[class*="st-key-meth_hero"]` in [theme.py](portal/theme.py) (`.bm-meth-hero` now only sets `color:#fff`). Button restyled **modular + bolder**: white bg, no border, `font-weight:800`, `border-radius:10px`, drop shadow, hover lift. (Old `st-key-meth_manual` rule replaced.)
- **Deviation-graph note moved beside the heading** ([app.py](portal/app.py) `page_performance`): the "Top/positive bars…" explainer now renders inline (flex row, `align-items:baseline`) to the right of the "Actual price vs Forecast Price Deviation Graph" `bm-h` heading instead of on its own line below it.
- **Weekly delta accuracy bar hover reverted** ([app.py](portal/app.py) `delta_acc_bar`): back to "Move captured: X%". The "Forecasted price change vs actual price change" phrasing instead now leads the **delta ⓘ tooltip** (`ACC_GLOSSARY["delta"]` idea) — shown on the "Weekly delta accuracy" heading + Delta-accuracy KPI ⓘ + glossary.

### 2026-07-24 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — User manual co-branded (BigMint × Adani), website + demo page removed
- Regenerated **`portal/assets/BigMint_Portal_User_Manual.pdf`** (now **4 pp**, was 5). Generator = `build_manual.py` in this session's scratchpad (reportlab); re-run if tour copy changes. No app code change — the Methodology download button already serves this file.
- **Co-branding:** Adani wordmark (`assets/adani_logo.png`, colour gradient) added in a **white chip top-right of the cover** (chip needed — the gradient wouldn't read on blue) and beside the BigMint logo in every content-page header. Cover subtitle + content header text now read **"BigMint · AI Labs  ×  Adani"**. New `adani_chip()` helper in the generator.
- **Website removed:** dropped the cover's gray `www.bigmint.co` band and the footer `www.bigmint.co` (footer keeps only `pg. N`).
- **Demo page removed:** deleted the whole back cover (Contact Us + "Book a Demo" button) — its `back()` fn, its `PageTemplate`, and the final `NextPageTemplate("back")`/`PageBreak`. Manual now ends on the "You're all set" closer.

### 2026-07-24 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology flowchart/manual + Performance chart-title/hover polish
- **Methodology flowchart** ([app.py](portal/app.py) `page_methodology`): image shrunk — `width:100%` → `width:60%;max-width:600px`, container `text-align:center` + img `display:inline-block` (centred).
- **Manual download button moved** into the "How the forecast is built" section (right after the `bm-meth-hero`, before the stat row); removed from page bottom. Text shortened "Download the portal user manual (PDF)" → **"Download user manual"**. Styled **white bg / blue text+border** via new scoped rule `div[class*="st-key-meth_manual"] .stDownloadButton button` in [theme.py](portal/theme.py); button wrapped in `st.container(key="meth_manual")`.
- **Performance chart titles** ([app.py](portal/app.py) `page_performance`): "Actual vs Forecast Price" → **"Actual vs Forecast Price Trend graph"**; "Actual price vs Forecast price" → **"Actual price vs Forecast Price Deviation Graph"**.
- **Top/positive-bars explainer** moved to a subtext line directly under the Deviation-graph heading (before `delta_bar(view)`, `margin-top:-4px`) instead of below the chart.
- **"Absent weeks … NA" footnote** made bolder (`font-weight:700`, `color:var(--bm-ink)`) and pulled tight to the chart (`margin-top:-2px`).
- **Weekly delta accuracy hover** ([app.py](portal/app.py) `delta_acc_bar`): "Move captured: X%" → **"Forecasted price change vs actual price change: X%"**.
...
### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Headings + spacing polish for readability
- **Section heading `.bm-h`** ([theme.py](portal/theme.py)) — app-wide: `15px/600` → `16.5px/700`, `margin 6px` → `18px 0 11px` + slight letter-spacing, so section titles are more prominent and separated.
- **Engine block headers `.bm-fm-hd`** — bigger number badge (24→26px), header label `12→12.5px` with wider tracking (`.09em`) and brighter white; more gap to the cards (`margin-bottom 14→17`, cards `gap 8→10`).
- **Engine column headers + chips** (scoped `.bm-engine-wide`): "INPUTS/OUTPUTS" labels darker (`--bm-primary-dark`) + wider tracking + more space below; chips `margin-bottom 10`, roomier padding; whole engine `margin 24px 0 20px` and column gap `14→18` for breathing room. All scoped so the calculators' `.bm-engine`/`.bm-chip` are unchanged.

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology engine: Inputs/Outputs chips now two-line (title + descriptor)
- **`engine_in`/`engine_out` chips gained a descriptor sub-line** ([app.py](portal/app.py) `page_methodology()`): each chip is now `<b>title</b>` + `<small>detail</small>` (e.g. "12-week forward price path" / "Weekly outlook, refreshed every week"). Local `_engine_chips` builder + tuples updated to 3 fields (icon, title, sub).
- **New scoped CSS `.bm-chip-txt`** ([theme.py](portal/theme.py)) for the two-line layout + muted `small`. Calculators' plain `.bm-chip` chips don't use `.bm-chip-txt`, so they're unchanged.

### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Login: topbar restored, white frost overlay (was blue)
- **Topbar back on the sign-in screen** ([app.py](portal/app.py) `login_screen()`): re-added `theme.render_topbar(None)` (same bar as elsewhere; earlier removal reverted). Footer stays off.
- **Backdrop overlay blue → white frost** (`_login_bg_style()`): the brand-tinted dark-blue gradient over the steel photo is now `linear-gradient(rgba(255,255,255,.80) → .66)` — the steel reads as a light industrial texture. Dropped the dark card-shadow bump (it was tuned for the dark overlay); the card uses LOGIN_CSS's lighter shadow again.

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Adani co-brand logo: white pill so it's visible on the blue bar
- **`.bm-adani-chip`** ([theme.py](portal/theme.py)) got `background:#fff; border-radius:8px; padding:5px 11px`. The Adani wordmark is a blue→magenta gradient whose blue half blends into the blue topbar — a white pill (standard for partner logos on coloured headers) guarantees contrast. A different bar-blue can't fix a logo that itself contains blue.
...
### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology engine: equal-height blue blocks, white cards, wider I/O columns
- **`.bm-fm` blue blocks now equal height** ([theme.py](portal/theme.py)): row `align-items:stretch` + `.bm-fm-cards{flex:1;justify-content:center}` so Market Inputs / AI Engine / Output are the same height with their cards vertically centred.
- **Nested cards white, not translucent blue**: `.bm-fm-card` and `.bm-fm-fc` → `background:#fff` + `var(--bm-border)`, text `var(--bm-primary-dark)` / `{NEUTRAL}`, icons `var(--bm-primary-soft)`/`var(--bm-primary)`. Blocks stay blue gradient; forecast keeps its red bottom border + "PREDICTION" badge.
- **Inputs/Outputs columns widened**: `.bm-engine-wide` grid `auto auto auto auto auto` → `260px auto auto auto 260px` (back to the old wider chip columns).

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology engine: modular infographic as the CENTRE, Inputs/Outputs columns kept
- **Restored the engine's Inputs & Outputs chip columns and put the blue `.bm-fm` infographic in the middle** (where the old plain "model" box was) — per user: "replace the blue box only with this, keep input and output as is." [app.py](portal/app.py) `page_methodology()` now builds `<div class='bm-engine bm-engine-wide'>` = Inputs column → arrow → full `.bm-fm` pipeline (Market Inputs / AI Engine / Output) → arrow → Outputs column. Section title back to **"From data to forecast"**. (User accepted that the centre repeats the flanking Inputs/Outputs at a higher detail.)
- **New CSS variant `.bm-engine-wide`** ([theme.py](portal/theme.py)): `grid-template-columns:auto auto auto auto auto; justify-content/align-items:center` so all tracks size to content and the short chip columns centre against the taller infographic. Plain `.bm-engine` grid unchanged (calculators still narrow). `.bm-engine-wide .bm-fm{margin:0}`.
- **Tour** ([tour.py](portal/tour.py)): pipeline step targets `.bm-engine-wide` (falls back to `.bm-fm`), desc covers Inputs → model → Outputs.

### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology: drop old engine, single blue Forecasting Model infographic
- **Reversed "keep both" — now one infographic.** Removed the old `.bm-engine` "From data to forecast" infographic (Inputs→blue core→Outputs) + its section title from [app.py](portal/app.py) `page_methodology()`. The page now has a single **"Forecasting Model"** section holding the modular `.bm-fm` pipeline. (`.bm-engine*` CSS stays in theme.py — still used by the three calculators.)
- **`.bm-fm` re-themed blue** ([theme.py](portal/theme.py)): blocks are now a blue gradient (`--bm-primary → --bm-primary-dark`) with translucent-white module cards, white text, white number badges (blue digits); the Forecast card is a brighter translucent-white card with the red bottom border + "PREDICTION" badge; arrows stay red for pop. Matches the old blue core box the user pointed at.
- **Tour anchor** ([tour.py](portal/tour.py)): "pipeline" step repointed `.bm-engine` → `.bm-fm` (the old engine is gone from Methodology).

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Forecasting Model infographic: blocks hug content (no full-width span)
- **`.bm-fm*` no longer stretches** ([theme.py](portal/theme.py)). Was full-width (`flex:1 1 0`) + equal-height (`align-items:stretch`), which left big empty areas in the AI-Engine/Output blocks. Now: row `align-items:center`, blocks `flex:0 0 auto; width:fit-content` so each wraps only its nested cards; cards fixed `width:210px`; dropped the `flex:1`/`justify-content:center` card fill and the 78px arrow offset (arrows just centre on the row now). Shorter blocks (AI Engine, Output) centre vertically against Market Inputs. Tablet media query drops the old `width:100%` block override so blocks stay content-sized when stacked.

### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology: keep BOTH infographics (old engine + new modular)
- **Reverted the "replace" from the earlier entry — now both live on the Methodology page** ([app.py](portal/app.py) `page_methodology()`):
  - **"From data to forecast"** → the original `.bm-engine` Inputs→model→Outputs infographic, restored verbatim (`engine_in`/`engine_out`/`_engine_chips`).
  - **New section "Forecasting Model"** (below it, before "Key factors") → the modular 3-block `.bm-fm` pipeline (Market Inputs → AI Forecast Engine → Output).
- **Tour anchor reverted** ([tour.py](portal/tour.py)): "pipeline" step points back to `.bm-engine` (original desc). The new Forecasting Model section is not a tour step.
- Both CSS families (`.bm-engine*` and `.bm-fm*`) already coexist in `theme.py`; no CSS change needed.
...
- **Sign-in screen** ([app.py](portal/app.py) `login_screen()`): removed `theme.render_topbar(None)` and `theme.footer()` — no brand top bar, no copyright footer on login. `force_password_change()` keeps both (unchanged).

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Login subtitle trimmed
- **Login page subtitle** ([app.py](portal/app.py) `_login_brand` call): "AI Labs · sign in to your dashboard" → **"Sign in to your dashboard"**.

### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Methodology "From data to forecast" infographic rebuilt (modular 3-block)
- **Replaced the old `.bm-engine` Inputs→model→Outputs infographic on the Methodology page** with a modular 3-block pipeline: **Market Inputs** (Domestic Price, Global Price, Supply & Demand, Cost Drivers, Macro Factors) → **AI Forecast Engine** (ML Evaluation, Sentiment Analysis, Impact Analysis) → **Output** (Forecast card, "PREDICTION" badge). Three uniform framed blocks, each with a numbered header and nested icon module cards; connector arrows offset past the fixed 62px header so they align to the card centre. New CSS `.bm-fm*` in [theme.py](portal/theme.py) (collapses to one column under 1024px); builder in [app.py](portal/app.py) `page_methodology()` (`fm_inputs`/`fm_engine` lists + `_fm_cards`). Section title stays **"From data to forecast"**.
- **`.bm-engine*` CSS kept** — still used by the three calculators' methodology panels (`calc_cost` / `calc_import_price` / `calc_elasticity`). Only the Methodology-page usage moved to `.bm-fm`.
- **4 new icon paths** in `theme.py` `_ICON_PATHS`: `globe`, `cpu`, `chat`, `monitor`.
- **Tour anchor updated** ([tour.py](portal/tour.py)): the Methodology "pipeline" step now targets `.bm-fm` (was `.bm-engine`).

### 2026-07-23 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++++) — HR Plate chart: China duty-paid landed-cost overlay
- **HR Plate now gets the violet "China landed" overlay** like HRC/Rebar IF Mumbai. Added a **`HR Plate` sheet** to `landed_costs.xlsx` (private data repo) — 132 weekly rows, 2024-01-07 → 2026-07-19, week-end dated, **ex-IGST** INR/t rounded to 50, built from the user's `HRPlate_China_landed_cost.xlsx` `Landed Cost` sheet col "Landed Cost, ex-IGST (INR/t)" (FOB Rizhao 5-10mm SS400 → India duty-paid). Wired via one entry in [app.py](portal/app.py) `_LANDED_OVERLAY` (`"HR Plate": ("HR Plate", "China landed")`). **Requires the data repo to be pushed + app cache cleared** (private data is fetched once per deploy).

### 2026-07-23 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++++) — Sensitivity: shocks really go to ±50 now (backend clamp fixed); Take-a-tour button white/blue, bigger+bolder
- **±50 clamp — actual fix.** The **2026-07-22** entry widened the slider/presets to ±50 but the shock still capped at ±20 because [calc_elasticity.py](portal/calculators/calc_elasticity.py) `_snap()` clamped to `[-20, 20]`. Widened `_snap` to `[-50, 50]` (matches the slider/number/preset range) — this is the single choke point every input mode routes through (`_cb_slider`/`_cb_number`/`_cb_preset` + Table editor all call `_snap`), so shocks up to ±50% now apply. Also fixed the Table "Shock %" help text (`−20 to +20` → `−50 to +50`).
- **Take-a-tour button restyled** ([tour.py](portal/tour.py) `#bm-tour-launch` CSS): solid **white** background with **blue (`#024CA1`) text**, **bigger** (`padding:12px 26px`, `font-size:15.5px`) and **bolder** (`font-weight:800`). Hover inverts to blue bg / white text.

### 2026-07-22 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++++) — Take-a-tour button: stop it vanishing + dock it in the blue bar next to Log out
- **Disappearing fixed with a MutationObserver.** [tour.py](portal/tour.py) `CONTROLLER`: the launcher only ran once, so any Streamlit rerun that re-rendered the topbar (React re-setting the node) wiped the injected button for good. Added `window.__bmTourObserver` (observes `document.body` childList/subtree) that calls `launcher()` whenever `#bm-tour-launch` is missing (launcher self-guards on the id → no loop).
- **Docked back inside the blue bar's right slot** (`.bm-topbar-r`) instead of a separate white-column host — so the **blue header now extends right up to the button**, which sits at the bar's right end just left of **Log out**. Reverted the 3-column header in [app.py](portal/app.py) back to `st.columns([6, 1])` and removed the `.bm-tour-host` markdown + `_tour_on` conditional.
- **Restyled to stay visible ON the blue bar:** translucent-white chip (`rgba(255,255,255,.15)` + `.5` white border, white text, radius 8), hover inverts to solid white / blue text (mirrors the Log-out invert). (Was a solid blue button that would have blended into the bar.)

### 2026-07-22 (latest++++++++++++++++++++++++++++++++++++++++++++++++++++) — Landed-cost chart stacked + gradient back; admin location CRUD via the table
- **Landed-cost chart is now STACKED with the diverging gradient restored** ([calc_import_price.py](portal/calculators/calc_import_price.py) `_landed_figure`): per origin, base = **Landed (Excl. duties)** + top = **all duties** = total landed. Both segments keep the **colour-by-distance-from-domestic** scale (green cheaper / amber parity / red pricier, `cmid=0`, symmetric `cmin/cmax`); the base uses a **lighter tint** (`LIGHT` scale) so the excl-duties part stands out. Base labels its value inside, the top labels the grand total outside; domestic dashed line kept; caption updated to explain the stack. (Replaces the brief grouped-bar version.)
- **Admin can add / remove / rename origins through the scenario table.** For `is_admin` (reached via **Admin tab → "Admin — Landed Cost defaults"**, `app.py:1537` `render(is_admin=True)`), the editor now shows **Location as an editable text column** with **`num_rows="dynamic"`** (＋ add row, 🗑 delete). The live origin set lives in `st.session_state["adm_region_list"]` (seeded from defaults); **Calculate** reconciles the edited frame → rebuilds the region list + per-origin session state (blank/duplicate names skipped, blank cells fall back to `_NEW_LOC` seeds, empty table falls back to defaults) and bumps the editor key to avoid ghost add/delete deltas. Non-admins are unchanged (fixed rows, Location as read-only index). Admin **pending/dirty** are forced `True` (dynamic rows make the row-diff moot).
- **Saved defaults are now authoritative** ([calc_import_price.py](portal/calculators/calc_import_price.py) `_effective_defaults`): when a saved `locations` block exists it **replaces** the built-in set (was: merge-over-existing-only), so an admin's **added/removed** origins persist via **💾 Save as default** (writes `db.set_setting(SETTINGS_KEY)`), not just value tweaks. New consts `_NEW_LOC` / `_LOC_FIELDS`. `py_compile` clean.
- **Fix (same day):** the scenario editor's `hide_index` was inverted (`not is_admin`), which **hid the Location names for regular users** (Location is their row index). Corrected to `hide_index=is_admin` — users see Location as the index again; admins keep it as the editable column.

### 2026-07-22 (latest+++++++++++++++++++++++++++++++++++++++++++++++++++) — Header uniform, tour button = logout design, footer re-brand, video button gated, misc tab polish
- **Uniform topbar title everywhere.** `DEFAULT_PROFILE["title"]` → **"GCP Steel - AI Labs - BigMint - Steel Price Forecasting Model"**; removed the per-role `title` overrides from `ROLE_PROFILES` (Adani/Analyst/Admin now inherit DEFAULT) — one header on every role + login. ([theme.py](portal/theme.py))
- **"Take a tour" button now matches the Log out button's design** — rounded-rectangle (`border-radius:8px`, `padding:12px 22px`, `font-weight:600`, subtle shadow) instead of a pill. Kept **blue** (per the standing "blue not orange" instruction); hover darker blue. ([tour.py](portal/tour.py) `#bm-tour-launch` CSS)
- **Zoom-button gradient extended to 26W + YTD.** [app.py](portal/app.py) `_HL_TEMPLATE` `.rangebtns` now shades `nth-child(5)=26W` `#023d82` and `(6)=YTD` `#012f66` (continuing the light-1W→dark ramp); ALL stays neutral.
- **Forecast-horizon selected tab = FULL orange fill** (not just the border). [theme.py](portal/theme.py): `.st-key-fc_horizon button[aria-checked="true"]` now sets `background:var(--bm-accent)` + white bold text (overrides the gradient bg + the generic light-orange-tint rule, which comes earlier).
- **Footer re-branded + reworded.** [theme.py](portal/theme.py) `footer()`: left span "AI-generated forecasts are indicative." → **"Note: AI-generated forecasts are indicative."**; right span "© BigMint · AI Labs | bigmint.co" (with link) → **"© BigMint · Adani"** (link dropped).
- **Analyst-call Video button gated on a backend link.** [app.py](portal/app.py) `_render_call_card`: the "Video Podcast" button now renders **only when `call["video"]` is set** (button-columns rebuilt without the video slot otherwise; the "· Video" kinds tag also drops when absent). Deleted the now-dead `_video_unavailable()` dialog; admin help text updated ("blank hides the button entirely").

### 2026-07-22 (latest++++++++++++++++++++++++++++++++++++++++++++++++++) — Calculators: sensitivity ±50%, landed-cost excl/incl-duties split
- **Price Sensitivity** ([calc_elasticity.py](portal/calculators/calc_elasticity.py)): title now **"Price Sensitivity Scenario Simulation"** + a small `(wrt Market Factor)` subtext span after it. Driver **sliders/number inputs extended to ±50%** (was ±20), with matching ticks + presets **-50 / -25 / 0 / +25 / +50**.
- **Import (Landed Cost)** ([calc_import_price.py](portal/calculators/calc_import_price.py)): title renamed **"Import Price Scenario Simulation" → "Import Landed Cost Price Scenario Simulation"**.
- **"Port" → "Port/Customs/Inland Freight"** everywhere it's a label: `DUTY_COLS["port_inr"]` header + its `data_editor` column_config (key/label/help) + the "Rupee landed" pipeline-step formula (`Landed = Cost$×FX + Port/Customs/Inland Freight`). Session-state field name stays `port_inr` (persistence/admin-save unaffected).
- **Editable scenario table: "Spot INR/t" column replaced by "Landed (Excl. duties)"** — derived, read-only, = **(FOB + Freight) × FX + Port/Customs/Inland Freight** (was FOB × FX). Caption + help updated. Not part of the pending/dirty diff (still a derived column).
- **Landed-cost chart is now GROUPED bars** ([calc_import_price.py](portal/calculators/calc_import_price.py) `_landed_figure`): per origin, **Landed (Excl. duties)** (light blue `#5E92D6`) vs **Landed incl. All duties** (primary blue), legend on, domestic-benchmark dashed line kept. Dropped the old single diverging-color bar. New `compute_landed()` return key **`landed_excl`** = `cfr × fx + port_inr` feeds the excl-duties bar.
- Chart y-axis / hovers now read **INR/MT** (carried over from the prior per-MT change). `py_compile` clean on both calculators.

### 2026-07-22 (latest+++++++++++++++++++++++++++++++++++++++++++++++++) — Login branding, tour button, week-tab gradients, per-MT axes, accuracy glossary
Batch of UI edits ([app.py](portal/app.py) / [theme.py](portal/theme.py) / [tour.py](portal/tour.py); `py_compile` clean on all three):
- **Login topbar now co-branded.** [theme.py](portal/theme.py) `render_topbar()` no longer blanks the co-brand chip when `user is None` — the login screen uses `DEFAULT_PROFILE` (which carries `adani_logo.png`), so it now reads **BigMint logo · | · Adani chip · | · title**. `DEFAULT_PROFILE["title"]` changed to **"GCP Steel - AI Lab - BigMint - Steel Price Forecasting Model"** (verbatim per request; only affects login + unknown roles — Adani/Analyst/Admin still override their own title).
- **"Take a tour" launcher: bigger, blue, docked beside Log out (outside the blue bar), always visible.** [app.py](portal/app.py) header: tour-eligible roles (`tour.TOUR_ROLES`) now get a 3-col header `[6, 1.5, 1]` with a **static markdown host `<div class='bm-tour-host'></div>`** between topbar and Log out; [tour.py](portal/tour.py) `launcher()` appends into `.bm-tour-host` (falls back to `.bm-topbar-r` → body). Static-markdown host (not `st.container`) so the JS-injected button **survives reruns** (React never re-sets identical innerHTML). CSS: blue fill `#024CA1` / white text, hover darker blue `#013a7d` (was white→orange), padding 11×22, 15px/800. Launcher was already always-appended + auto-start-once, so "always visible" held.
- **Week tabs → blue gradient (light 1W → dark 12W).** Two controls: the **forecast-forward horizon** segmented control (`fc_horizon`, options 1/4/8/12) via `.st-key-fc_horizon button:nth-of-type(1..4)` in [theme.py](portal/theme.py); and the **historical zoom buttons** (`.rangebtns` in [app.py](portal/app.py) `_HL_TEMPLATE`) via `:nth-child(1..4)` (26W/YTD/ALL stay neutral). Shades: `#e8f0fb / #b9d3f2 / #5b93da / #024CA1`. Selected state still shows: horizon keeps the generic orange border, zoom keeps `.active` orange (defined after the gradient so it wins on click).
- **"per MT" on every price y-axis.** [app.py](portal/app.py) `_style_fig()` money branch (covers forecast_chart + perf_chart) and `delta_bar()` get `title_text="per MT"` (11px grey, standoff 8) — reads with the existing `INR ` tick prefix as "INR 62,000 per MT". `_PERF_ML` **68 → 92** so the vertical title fits on the fixed-margin perf charts while all perf charts (incl. the % ones) stay left-aligned. Forecast chart keeps `automargin=True` so its title auto-fits. % charts (accuracy/directional/delta-acc) unchanged (not prices).
- **Accuracy glossary + hover tooltips (Performance page).** New `ACC_GLOSSARY` dict + `_acc_help(key)` (a `ⓘ` `.bm-help` span whose native `title=` carries idea+formula) + `_acc_glossary_html()` (a `.bm-glossary` box) in [app.py](portal/app.py). `ⓘ` markers added to the 3 KPI cards (MAPA/Directional/Delta) and the accuracy chart titles; the glossary box renders above the footer. Styles (`.bm-help`, `.bm-glossary*`) in [theme.py](portal/theme.py).
- **Bold commodity tab names.** `.st-key-fc_group / -perf_group / -fc_prod / -perf_prod button p { font-weight:800 }` in [theme.py](portal/theme.py).
- **Rename** "Actual vs forecast" → **"Actual vs Forecast Price"** (Performance `perf_chart` title + Forecasting tabular-view title).
- **Weeks-averaged on the KPI cards.** Directional + Delta accuracy card subs now append ` · {len(view)} wk` (matching the MAPA card).
- **Follow-up (same day):** (1) **Topbar order swapped** — Adani chip now sits BEFORE the BigMint logo (`render_topbar` builds `[Adani | ] BigMint | title`; Analyst/Admin still `BigMint | title`). (2) **"per MT" extended to the calculator charts** — the landed-cost bar ([calc_import_price.py](portal/calculators/calc_import_price.py) y-axis) and the contribution bar ([calc_elasticity.py](portal/calculators/calc_elasticity.py) x-axis) unit labels changed `INR/t` → **`INR/MT`**; the cost build-up chart ([calc_cost.py](portal/calculators/calc_cost.py)) already read `INR/MT`. Only the chart axes were touched — table/input/data `INR/t` labels left as-is.
...
### 2026-07-21 (latest++++++++++++++++++++++++++++++++++++++++++++++) — Scenario Simulation top spacing: REAL cause found — page-global block-gap override
- **Root cause (the one that actually mattered):** all three calculator tabs' `CALC_CSS` injected `[data-testid="stVerticalBlock"] { gap: 0.9rem !important; }` ([calc_elasticity.py](portal/calculators/calc_elasticity.py), [calc_import_price.py](portal/calculators/calc_import_price.py), [calc_cost.py](portal/calculators/calc_cost.py)). That selector is **page-global, not scoped** — so on the Scenario page every block gap (including nav → tab strip → content) was 0.9rem while every other page used the app-wide 0.65rem ([theme.py:190](portal/theme.py:190)). The whole page sat lower. No container/margin edit could fix it because they targeted the wrong element.
- **Fix:** removed the `stVerticalBlock` gap override from all three calculators (left the `stHorizontalBlock` 1.1rem column-air rule — it doesn't affect page-to-page top alignment). Scenario page now inherits the same 0.65rem gap as every page → tops match. `theme.py:190` is now the single source of the block gap. `py_compile` clean.

### 2026-07-21 (superseded — wrong element) — Scenario Simulation top spacing: rewritten to mirror page_forecasting
- **Root cause:** the margin-hack attempts never converged because the real culprit was the `st.container(key="bm_calc_top")` **wrapper** itself — it nested an extra `stVerticalBlock`, so no top-margin value lined the strip up with a bare control page.
- **Fix (reference = `page_forecasting`, the sibling that also opens on a control strip and looks right):** dropped the wrapper container entirely — `page_calculators` now calls `st.tabs(...)` directly, exactly like forecasting opens on its group segmented-control ([app.py:1757](portal/app.py:1757)). Deleted the dead `st-key-bm_calc_top` CSS rule from [theme.py](portal/theme.py). No hacks, no offsets — the strip now sits flush like forecasting's.

### 2026-07-21 (superseded above ×2) — Scenario Simulation top spacing: drop the 0.35rem push-down
- Interim: set `st-key-bm_calc_top{margin-top:0}`. Didn't converge — see root cause above.

### 2026-07-21 (superseded) — Scenario Simulation top spacing aligned with other pages
...
- Expanded the analyst tour ([tour.py](portal/tour.py) `STEPS`) from a single "whole page" step per section to **35 granular steps**. Each cross-page section now highlights its individual controls:
  ...
  - **Methodology** — hero (`.bm-meth-hero`), stat strip (`.bm-stat-row`), the "From data to forecast" engine (`.bm-engine.bm-engine-wide`: Inputs chips → blue modular `.bm-fm` pipeline → Outputs chips), key-factor grid (`.bm-factor-grid`), horizon block (`.bm-horizon`).

...
## Changelog
### 2026-07-11 (latest+++++) — Analyst calls: Gemini AI auto-fill from the pitch deck
- **New module `portal/ai_fill.py`.** `gemini_ready()` (key present?), `extract_pptx_text(bytes, name)` (pulls titles/bodies/tables/speaker-notes from a **.pptx** via `python-pptx`; raises on legacy `.ppt` or empty deck), and `fill_analyst_sections(deck_text, sections)` — POSTs to the Gemini **Generative Language API** (`{model}:generateContent`, default `gemini-2.0-flash`, JSON mode via `responseSchema`) and returns `{"summary": <short paragraph>, <section>: <one-liner>, …}`. Deck text capped at 20k chars; API key sent as the `x-goog-api-key` header (not a URL query).
- **Admin editor wiring** (`page_admin()`): the **Pitchdeck uploader moved out of the `st.form`** (above it) so one upload both attaches the deck on save *and* feeds the AI; added an **"✨ Auto-fill sections with AI"** button next to it (enabled only for a `.pptx` when a key is configured). On click it extracts text → calls Gemini → writes the drafted summary + section lines into the form fields' `session_state` keys → `st.rerun()` (admin reviews/edits before Save). Summary = short paragraph, each section = one-liner, per spec.
...
- **Secrets / deps.** API key read from **`st.secrets['gemini']['api_key']`** (git-ignored `secrets.toml`; **never hardcoded** — public repo). Added a `[gemini]` block to `secrets.toml` (real key, local) + `secrets.toml.example` (placeholder). Added **`python-pptx>=0.6.21`** to both `requirements.txt` files. On Streamlit Cloud, paste the `[gemini]` block into *Manage app > Settings > Secrets*. Local testing needs `pip install python-pptx`.

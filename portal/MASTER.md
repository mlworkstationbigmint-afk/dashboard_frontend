...
## Changelog
### 2026-07-16 (latest+++++++++++++++++) — Cost Head: stacked cost-element bars + per-plant market-price line
- **Chart rebuilt** in [calc_cost.py](portal/calculators/calc_cost.py) `_cost_margin_figure(names, edited, mkt_prices)` — now takes the per-plant edited tables + per-plant market prices (was `totals, margins, mkt_price`). Bars are **stacked**, one segment per cost element (segment = consumption norm × unit price, new `SEG_COLORS` 10-colour palette), so each bar shows every component's contribution; the **grand total is labelled above each bar** (layout `annotations`).
- **Mill-margin line removed** (the dotted diamond scatter + secondary y-axis are gone → single Rs./MT axis). Margin is still computed and shown in the per-plant `_totals_line` + banner/verdict.
- **Market price is now per-plant** — the single `st.number_input` became one input per plant (`cost_mkt_{key}_{n}`, `mkt_prices` dict) and is drawn as an **orange line+markers** across the plants instead of a single `add_hline`. Margins, banner (`mkt_prices[lower]`) and verdict (`mkt_prices[best]`, "current market prices") updated to use it.
...
- ⚠ Visual-only — verify in-app: 11-entry legend layout, total-label placement above tall bars, per-plant market inputs.

...
- **Seed display name** ([seed_users.py](portal/seed_users.py)): `("adani", "Adani User", "Adani")` → `("adani", "User", "Adani")`, so the Home greeting `Welcome, **{user['name']}**` ([app.py](portal/app.py) `page_home`) no longer says "Adani User" on fresh seeds. **Live DB caveat:** the greeting reads `name` from the Neon user store; the existing `adani` row still holds "Adani User" — rename it via **Admin → Manage a user** or re-seed for the running app to reflect this.
- **Deliberately KEPT** (per request): topbar co-brand Adani logo/chip + `© BigMint - Adani` footer ([theme.py](portal/theme.py)), and the `"Adani"` access-control **role** name (`auth.ROLES`, profiles, `GROUPED_FORECASTING_ROLES`) — role only shows in admin-only management screens, not to the Adani user.
